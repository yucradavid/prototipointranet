...
export default function SecretariaWorkbenchView({items,procedures,onRegisterVirtual,onCreatePhysical,onFinalize}){
  const [tab,setTab]=useState('entrada'),[selectedId,setSelectedId]=useState(null),[search,setSearch]=useState(''),[modal,setModal]=useState(false),[form,setForm]=useState(physicalBase)
  ...
  const submitPhysical=()=>{const p=procedureById(form.procedureId);if(!form.solicitante.trim())return;onCreatePhysical({...form,asunto:p.name,ownerProfile:form.condicion==='Docente'?'docente':'estudiante'});setModal(false);setForm(physicalBase)}
    ...
    <div className="master-detail-grid">
      ...
      <Panel title={selected?`${selected.numero?`EXP ${selected.numero}`:selected.tracking} · ${selected.asunto}`:'Detalle'} subtitle={selected?`${selected.solicitante} · ${selected.canal}`:'Selecciona un registro.'}>
        {selected?<div className="case-detail">
          ...
          {selected.estado==='SOLICITUD_VIRTUAL'&&<button className="btn primary full" onClick={()=>onRegisterVirtual(selected)}><ArrowRight size={17}/> Validar, generar N.° expediente y enviar a Dirección</button>}
