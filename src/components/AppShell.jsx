...
export default function AppShell({profileId,setProfileId,officeId,setOfficeId,offices,activeView,setActiveView,alerts=[],onReset,onLogout,children}){
  const [mobile,setMobile]=React.useState(false)
  const [notifOpen,setNotifOpen]=React.useState(false)
  const profile=PROFILES.find(x=>x.id===profileId)||PROFILES[0]
  const nav=profileId==='admin'?[['control','Centro de control',ShieldCheck],['workflow','Trámites y rutas',Workflow],['catalog','Oficinas y trámites',LayoutGrid],['book','Libro y auditoría',BookOpen],['tracking','Seguimiento global',Search]]
  ...
  return <div className="app-shell">
    ...
    <main className="main-area">
      <header className="topbar"><button className="hamb" onClick={()=>setMobile(true)}><Menu size={20}/></button><div className="topbar-context"><span>Perfil de demostración</span><b>{profile.label}{profile.office?` · ${officeName(profile.office)}`:''}</b></div><div className="topbar-right">
        ...
        <div className="profile-switch"><Icon size={18}/><select value={profileId} onChange={e=>{setProfileId(e.target.value);setActiveView(e.target.value==='admin'?'control':e.target.value==='estudiante'||e.target.value==='docente'?'portal':'work')}}>{PROFILES.map(p=><option value={p.id} key={p.id}>{p.label}</option>)}</select><ChevronDown size={15}/></div></div></header>
      {profileId==='oficina'&&<div className="office-context"><span>Oficina operativa para la demo:</span><select value={officeId} onChange={e=>setOfficeId(e.target.value)}>{officeOptions.map(o=><option value={o.id} key={o.id}>{o.name}</option>)}</select><small>En producción se obtiene del usuario autenticado; no se selecciona manualmente.</small></div>}
